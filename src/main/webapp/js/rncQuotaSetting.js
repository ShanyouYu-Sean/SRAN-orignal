...
$(document).ready(function() {
    ...
    $('.setting_btn > span:nth-child(1)').bind('click', function(){
        ...
        $.ajax({
            ...
            type:"post",
            url:"../lte/mod_rnc_quota_threshold",
...
function init()
{
    $("#rnc_quota_list li").not(":first").remove();
    $.ajax({
        contentType : 'application/json;charset=utf-8',
        type:"get",
        url:"../lte/get_rnc_quota_threshold",
        dataType:'json',
        success:function(response){
            //	console.log(response);
            if(response.status)
                ...
                $.each(response.data, function(i,item){
                    ...
                    if(i+1 >= start && i+1 <= end)
                    {
                        ...
                        var str2 = '<li class="link_setting" >'+
                            ...
                            '<div  align="center">' +
                            '<span onclick="rnc_mod(this)" class="setting_mod" style="float: none!important;">修改</span>' +
                            '</div>' +
                            '</li>'
                        $('#rnc_quota_list').append(str2);
                    }
                });
                sessionStorage.setItem("tac_quota_list", JSON.stringify(response.data));
...
function page(pageCount, current)
{
    ...
    $(".page").createPage({
        ...
        backFn:function(p){
            ...
            $("#rnc_quota_list li").not(":first").remove();
            var str = '';
            $.each(JSON.parse(sessionStorage.getItem("rnc_quota_list")),function(i,item){
