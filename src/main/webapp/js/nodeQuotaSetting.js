...
$(document).ready(function() {
    ...
    $('.setting_btn > span:nth-child(1)').bind('click', function(){
        ...
        $.ajax({
            ...
            type:"post",
            url:"../lte/mod_node_quota_threshold",
...
function init()
{
    $("#node_quota_list li").not(":first").remove();
    $.ajax({
        contentType : 'application/json;charset=utf-8',
        type:"get",
                ...
                $.each(response.data, function(i,item){
                    ...
                    if(i+1 >= start && i+1 <= end)
                    {
                        ...
                        var str2 = '<li class="link_setting" >'+
                            ...
                            '</div>' +
                            '</li>'
                        $('#node_quota_list').append(str2);
                    }
                });
                sessionStorage.setItem("node_quota_list", JSON.stringify(response.data));
...
function page(pageCount, current)
{
    ...
    $(".page").createPage({
        ...
        backFn:function(p){
            ...
            $("#node_quota_list li").not(":first").remove();
            var str = '';
            $.each(JSON.parse(sessionStorage.getItem("node_quota_list")),function(i,item){
