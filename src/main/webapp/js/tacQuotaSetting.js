...
$(document).ready(function() {
    ...
    $('.setting_btn > span:nth-child(1)').bind('click', function(){
        ...
        $.ajax({
            ...
            type:"post",
            url:"../lte/mod_tac_quota_threshold",
...
function init()
{
    $("#tac_quota_list li").not(":first").remove();
    $.ajax({
        contentType : 'application/json;charset=utf-8',
        type:"get",
...
function page(pageCount, current)
{
    ...
    $(".page").createPage({
        ...
        backFn:function(p){
            ...
            $("#tac_quota_list li").not(":first").remove();
            var str = '';
            $.each(JSON.parse(sessionStorage.getItem("tac_quota_list")),function(i,item){
